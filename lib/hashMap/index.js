...
class HashMap {
  ...
  clear() {
    const capacity = this.#capacity = 8;
    this.#isBitwise = true;
    this.#size = 0;
  ...
  values() {
    const self = this;
    return (function* () {
      for (const el of self) yield el[1];
    })();
  }
  ...
  keys() {
    const self = this;
    return (function* () {
      for (const el of self) yield el[0];
    })();
  }

  forEach(cb) {
    let i = 0;
    for (const el of this) cb(el, i, this);
  }
